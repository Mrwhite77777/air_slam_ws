...
DEFAULT_INPUT_PATH = os.path.join(PROJECT_SRC_DIR, "datasets", "trajectory_v1.txt")


def load_positions(input_path):
    """
    读取轨迹文件，并提取第 2 到第 4 列作为位置坐标。
    """
    ...
    with open(input_path, "r", encoding="utf-8") as file:
        for line_number, raw_line in enumerate(file, start=1):
            ...
            if len(parts) < 4:
                raise ValueError(f"第 {line_number} 行列数不足，无法提取第 2 到第 4 列: {line}")
...
def plot_trajectory(positions, output_path=None):
    ...
    axis_2d.scatter(x_values[0], y_values[0], color="green", s=40, label="Start")
    axis_2d.scatter(x_values[-1], y_values[-1], color="red", s=40, label="End")
    axis_2d.set_title("Trajectory XY")
    ...
    axis_3d.set_title("Trajectory XYZ")
...
def parse_args():
    ...
    解析命令行参数。
    """
    parser = argparse.ArgumentParser(description="读取 trajectory_v0.txt 的第 2 到第 4 列并绘制轨迹图")
    parser.add_argument(
        "--input",
